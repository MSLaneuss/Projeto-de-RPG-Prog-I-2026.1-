...
def gerar_sala(estado, dados):
    ...
    if   progresso <= 0.20: nivel_monstro = 0
    elif progresso <= 0.40: nivel_monstro = 1
...
def sortear_item(ui, estado, dados):
    ...
    if random.random() < 0.80:
        ...
        if categoria in ("armas", "armaduras"):
            itens_possiveis = [i for i in itens_possiveis if i["nome"] not in nomes_possuidos]
        ...
        tesouros_disponiveis = [t for t in tesouros if t["nome"] not in estado["tesouro coletado"]]
        if tesouros_disponiveis:
            item_sorteado = random.choice(tesouros_disponiveis)
            estado["tesouro coletado"].append(item_sorteado["nome"])
        else:
            item_sorteado = random.choice(dados["dados de interação"]["poção"])
