...
def gerar_masmorra(estado_jogo):
    ...
    if progresso <= 0.20:
        lvl_max = 0
    elif progresso > 0.20 and progresso <= 0.40:
        lvl_max = 1
    elif progresso > 0.40 and progresso <= 0.50:
        lvl_max = 2
            ...
            if sala["tipo"] == "combate":
                ...
                if estado_jogo["sala atual"] <= estado_jogo["sala totais"]:
                    if random.random() < 0.80:
                        ...
                        if categoria == "armas" or categoria == "armaduras":
                            itens_possiveis = [i for i in itens_possiveis if i["nome"] not in itens_adquiridos]
                        if itens_possiveis:
                        ...
                        tesouro_disponivel = [t for t in tesouros if t["nome"] not in estado_jogo["tesouro coletado"]]
